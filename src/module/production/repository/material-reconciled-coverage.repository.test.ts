...
beforeEach(()=>{vi.clearAllMocks();disposition='CARRY';m.connect.mockResolvedValue(tx);
  const base={article_id:'article',operation_id:'cut',unit:'u',supply_mode:'PURCHASE',requirements,debit_rule:rule,required_qty:100,specification_reviewed_at:'reviewed',technical_hash:'hash',row_version:1,designation:'Plat alu',supplier_id:'supplier'};
  tx.query.mockImplementation(async(sql:string)=>({rows:
    ...
    sql.startsWith('SELECT a.id::text,a.code,a.unite')?[{id:'article',code:'ALU',unite:'u'}]:[]
  }));
});
