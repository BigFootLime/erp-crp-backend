...
export type FutureMaterialSupply={id:string;commandId:string;code:string;status:string;articleId:string;designation:string;unit:string|null;purchaseUnit:string|null;
  ...
  requirements:Array<{type:string;valeur:string;obligatoire:boolean}>;documents:string[];version:string;allocatedNeedIds:string[]};
...
export async function readFutureMaterialSupplyTx(tx:DossierDb,articleIds:string[]){
  const rows=(await tx.query<FutureMaterialSupply>(`SELECT l.id::text,c.id::text AS "commandId",c.code,c.statut AS status,l.article_id::text AS "articleId",l.designation,
    ...
    COALESCE(b.assigned,0)::float8 AS assigned,COALESCE(r.received,0)::float8 AS received,COALESCE(l.magasin_id,c.magasin_livraison_id)::text AS "destinationId",
    ...
    LEFT JOIN LATERAL(SELECT sum(CASE WHEN besoin_type='OF_MATERIAL' THEN quantite_couverte ELSE quantite_couverte*COALESCE(l.coef_conversion,1) END) AS assigned,
      array_agg(material_need_id::text) FILTER(WHERE material_need_id IS NOT NULL) AS need_ids FROM public.commande_fournisseur_ligne_besoin WHERE ligne_id=l.id AND NOT annule) b ON true
...
export function futureSupplyCompatibility(need:{id:string|null;articleId:string|null;unit:string|null;destinationId:string|null;supplyMode:string;requirements:MaterialRequirements},source:Awaited<ReturnType<typeof readFutureMaterialSupplyTx>>[number]){
  ...
  // Do not retroactively attribute received, potentially refused material. Its
  // disposition must be decided through the receipt/quality workflow first.
  if(source.unassignedReceived>0)reasons.push('Une réception non affectée existe : terminez sa mise en stock avant d’attribuer le solde futur.');
  return reasons;
}
...
export async function allocateFutureMaterialSupplyTx(tx:PoolClient,input:{lineId:string;needId:string;sourceRef:string;ofId:number;quantity:number}){
  ...
  await tx.query(`INSERT INTO public.commande_fournisseur_ligne_besoin(ligne_id,besoin_type,besoin_ref,besoin_of_id,of_id,quantite_couverte,material_need_id)
    VALUES($1::uuid,'OF_MATERIAL',$2,$3,$3,$4,$5::uuid)`,[input.lineId,input.sourceRef,input.ofId,input.quantity,input.needId]);
}
