import {coverageFingerprint,type DebitRule,type MaterialRequirements} from './of-material';
...
export function materialCarryBlockers(previous:ReconciliationIdentity,target:ReconciliationIdentity):string[]{
  ...
  if(coverageFingerprint(previous.requirements)!==coverageFingerprint(target.requirements))blockers.push('Les exigences matière diffèrent ; faites traiter les engagements existants par les méthodes et les achats.');
  if(previous.consumed>0&&(previous.operationId!==target.operationId||coverageFingerprint(previous.debitRule)!==coverageFingerprint(target.debitRule)))
