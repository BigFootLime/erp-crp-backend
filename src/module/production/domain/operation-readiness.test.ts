...
describe("autorisation par opération et quantité",()=>{
  it('un retour complet exige encore le transfert physique explicite',()=>expect(evaluateOperationReadiness({...base,materials:[],predecessors:[{id:'external',label:'Traitement',good:100,done:true,transferred:0,partial:false,minimum:1,requireTransfer:true}]}).canStart).toBe(false));
  it('40 retours transférés autorisent exactement 40 pièces si la gamme permet le partiel',()=>expect(evaluateOperationReadiness({...base,materials:[],predecessors:[{id:'external',label:'Traitement',good:40,done:false,transferred:40,partial:true,minimum:20,requireTransfer:true}]})).toMatchObject({canStart:true,availableQuantity:40}));
  it('40 transférées restent bloquées si la gamme exige les 100 pièces',()=>expect(evaluateOperationReadiness({...base,materials:[],predecessors:[{id:'external',label:'Traitement',good:100,done:true,transferred:40,partial:false,minimum:1,requireTransfer:true}]}).canStart).toBe(false));
