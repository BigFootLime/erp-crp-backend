...
export function futureSupplyBalance(input:{ordered:number;cancelled:number;coefficient:number;assigned:number;received:number}){
  if([input.ordered,input.cancelled,input.coefficient,input.assigned,input.received].some(n=>!Number.isFinite(n)||n<0)||input.coefficient<=0||input.cancelled>input.ordered)
