...
export async function createRecursiveOrdresFabrication(tx: Queryable, params: {
  ...
  planned_quantities_by_path?: Record<string, number>;
  /**
   * Reuses the traceable root placeholder created from a customer order while
   * its technical file was incomplete. Children are still generated by this
   * unique recursive engine; the commercial order never gets a duplicate root
  ...
  const plannedQuantities = params.planned_quantities_by_path ?? null;
  const tree = plannedQuantities
    ? fullTree.filter((node) => Number(plannedQuantities[node.key] ?? 0) > 1e-9)
    : fullTree;
  if (tree.length === 0 || tree[0]?.level !== 0) {
    throw new HttpError(422, "OF_GENERATION_PLAN_EMPTY", "Le plan de fabrication ne contient aucun OF racine à créer.");
  }
  const activeKeys = new Set(tree.map((node) => node.key));
  const orphan = tree.find((node) => node.parent_key && !activeKeys.has(node.parent_key));
  if (orphan) {
    throw new HttpError(422, "OF_GENERATION_PLAN_ORPHAN", `Le sous-OF ${orphan.code_piece} n'a pas d'OF parent actif.`);
  }
  ...
  const technicalSourceHash = computeOfSourceHash(tree, technicalByKey);
  // Preserve the legacy SIMPLE/manual hash byte-for-byte.  An assembly plan
  // adds its exact net requirements to the source hash so stale stock/BOM
  // decisions cannot be confirmed after the preview.
  const sourceHash = plannedQuantities
    ? crypto.createHash("sha256").update(JSON.stringify({
        technical: technicalSourceHash,
        planned_quantities_by_path: plannedQuantities,
      })).digest("hex")
    : technicalSourceHash;

  const existingRoot = params.existing_root_of_id
    ? (await tx.query<{
        id: number;
        numero: string;
        ...
        `SELECT id::bigint::int AS id, numero, generation_batch_id::text AS generation_batch_id,
                statut::text AS statut, generation_level::int AS generation_level
           FROM public.ordres_fabrication
          WHERE id = $1::bigint
          FOR UPDATE`,
        [params.existing_root_of_id]
      )).rows[0] ?? null
    : null;
  if (params.existing_root_of_id && !existingRoot) {
    throw new HttpError(404, "OF_NOT_FOUND", "L'OF brouillon à compléter n'existe plus.");
  }
  if (existingRoot && (existingRoot.statut !== "BROUILLON" || existingRoot.generation_level !== 0)) {
    throw new HttpError(409, "OF_ROOT_NOT_REUSABLE", "Seul un OF racine brouillon peut recevoir le dossier technique validé.");
  }

  const ofIdByKey = new Map<string, number>();
  for (const node of tree) {
    if (node.level === 0 && existingRoot) {
      ofIdByKey.set(node.key, existingRoot.id);
    } else {
  ...
  const batchId = existingRoot?.generation_batch_id ?? crypto.randomUUID();
  ...
  if (existingRoot) {
    if (!existingRoot.generation_batch_id) {
      throw new HttpError(409, "OF_GENERATION_BATCH_MISSING", "Le lot de génération de l'OF brouillon est introuvable.");
    }
    await tx.query(
      `UPDATE public.of_generation_batches
          SET source_type = $2,
              commande_id = $3::bigint,
              commande_ligne_id = $4::bigint,
              affaire_id = $5::bigint,
              root_of_id = $6::bigint,
              root_piece_technique_id = $7::uuid,
              requested_qty = $8,
              metadata = COALESCE(metadata, '{}'::jsonb) || $9::jsonb,
              idempotency_key = COALESCE(idempotency_key, $10),
              request_hash = COALESCE(request_hash, $11),
              source_hash = $12
        WHERE id = $1::uuid`,
      [
        batchId,
  ...
  for (const node of tree) {
    ...
    const qtyLancee = Number((plannedQuantities
      ? Number(plannedQuantities[node.key] ?? 0)
      : params.qty_to_produce * node.quantite_cumulee).toFixed(3));
    ...
    if (reusesExistingRoot) {
      await tx.query(
        `UPDATE public.ordres_fabrication
            SET affaire_id = $2::bigint,
                commande_id = $3::bigint,
                commande_ligne_id = $4::bigint,
                article_id = $5::uuid,
                client_id = $6,
                piece_technique_id = $7::uuid,
                piece_technique_version_id = $8::uuid,
                technical_snapshot = $9::jsonb,
                technical_snapshot_sha256 = $10,
                technical_snapshot_at = now(),
                technical_readiness = 'VALIDATED',
                technical_validated_at = now(),
                technical_validated_by = $16,
                technical_preparation = COALESCE(technical_preparation, '{}'::jsonb)
                  || jsonb_build_object('completed_from_draft', true),
                parent_of_id = NULL,
                root_of_id = $1::bigint,
                generation_batch_id = $11::uuid,
                generation_level = 0,
                source_bom_line_id = NULL,
                structure_path = $12,
                quantity_per_parent = $13,
                quantity_cumulative = $14,
                quantite_lancee = $15,
                updated_at = now(),
                updated_by = $16
          WHERE id = $1::bigint`,
