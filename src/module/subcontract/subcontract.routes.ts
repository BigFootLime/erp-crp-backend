import { assertReceiptProcessingClosed } from "../receptions/repository/receipt-processing-guard";
import {assertMaterialOperationStartTx} from '../production/repository/operation-readiness.repository';
...
async function ledger(eventType: "ISSUE" | "RETURN", req: any, res: any, next: any) { try {
  ...
  const c=await pool.connect(); try { await c.query("BEGIN");
    const flowInstalled=await subcontractFlowInstalled(c);
    if(flowInstalled)await c.query('SELECT revision FROM public.planning_central_settings WHERE singleton FOR UPDATE');
    await c.query('SELECT pg_advisory_xact_lock(hashtextextended($1,0))',['subcontract-ledger:'+key]);
    ...
    const lots=await c.query<any>("SELECT lot_status FROM public.lots WHERE id=$1::uuid FOR UPDATE",[lot]); const expected=eventType==="ISSUE"?"LIBERE":"QUARANTAINE";
    ...
    if(eventType==='ISSUE'){
      ...
      if(readiness&&issued.rows[0].qty+amount>readiness.operation.availableQuantity+readiness.operation.processedQuantity)throw new HttpError(409,'SUBCONTRACT_PREDECESSOR_QUANTITY','La quantité expédiée dépasse les pièces disponibles à cette étape.');
      if(issued.rows[0].qty+amount>Number(p.qty_planned))throw new HttpError(409,'SUBCONTRACT_OVER_ISSUE','La sortie dépasse la quantité prévue du dossier.');
    }
    const inserted=await c.query<any>(
      "INSERT INTO public.subcontract_work_package_ledger(package_id,event_type,lot_id,qty,unit,idempotency_key,created_by) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id",
      [packageId,eventType,lot,amount,eventUnit,key,req.user?.id??null]);
    await audit(c,req.user?.id,eventType,packageId!,{lot,qty:amount,unit:eventUnit,idempotency_key:key}); await c.query("COMMIT"); res.status(201).json({package_id:packageId,ledger_event_id:inserted.rows[0].id,idempotent_replay:false});
  } catch(e) { await c.query("ROLLBACK"); throw e; } finally { c.release(); }
} catch(e) { next(e); } }
...
async function finalise(status:"CLOSED"|"CANCELLED",req:any,res:any,next:any){try{const packageId=id(req.params.id),expected=qty(req.body?.expected_row_version),reason=String(req.body?.reason??"").trim();required(packageId&&Number.isInteger(expected)&&expected>0&&reason,"expected_row_version et motif sont obligatoires.");const c=await pool.connect();try{await c.query("BEGIN");await lockRevision(c);const p=await lockPackage(c,packageId!);if(Number(p.row_version)!==expected)throw new HttpError(409,"SUBCONTRACT_CONCURRENT_OR_FINAL","Dossier modifié ou déjà finalisé.");const b=await c.query<any>("SELECT COALESCE(sum(qty) FILTER(WHERE event_type='ISSUE'),0)-COALESCE(sum(qty) FILTER(WHERE event_type='RETURN'),0) qty FROM public.subcontract_work_package_ledger WHERE package_id=$1::uuid",[packageId]);if(Number(b.rows[0].qty)!==0)throw new HttpError(409,"SUBCONTRACT_CUSTODY_OPEN","Finalisation refusée : matière toujours chez le sous-traitant.");await assertReceiptProcessingClosed(c,{orderLineId:p.supplier_order_line_id});const r=await c.query<any>("UPDATE public.subcontract_work_packages SET status=$2,row_version=row_version+1,closed_at=now(),closed_by=$3,close_reason=$4 WHERE id=$1::uuid AND status='SENT' AND row_version=$5 RETURNING *",[packageId,status,req.user?.id??null,reason,expected]);if(!r.rows[0])throw new HttpError(409,"SUBCONTRACT_CONCURRENT_OR_FINAL","Dossier modifié ou déjà finalisé.");await audit(c,req.user?.id,status,packageId!,{reason});await c.query("COMMIT");res.json(r.rows[0]);}catch(e){await c.query("ROLLBACK");throw e}finally{c.release()}}catch(e){next(e)}}
