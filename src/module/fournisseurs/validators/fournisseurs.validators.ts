...
export const createCatalogueSchema = z.object({
  body: z
    .object({
      ...
      lot_achat: z.number().finite().positive().optional().nullable(),
      unite_stock: z.string().trim().min(1).max(30).optional().nullable(),
      coef_conversion: z.number().finite().positive().optional().nullable(),
