...
export async function assertLegacyMaterialWrite(tx:Pick<PoolClient,"query">,ofId:number,articleId:string|null){
  ...
  const guarded=(await tx.query(`SELECT EXISTS(SELECT 1 FROM public.of_material_needs n WHERE n.of_id=$1 AND($2::uuid IS NULL OR n.article_id=$2::uuid))
    OR(EXISTS(SELECT 1 FROM public.app_feature_flags WHERE key='PRODUCTION_MATERIAL_WORKFLOW' AND enabled)
      AND EXISTS(SELECT 1 FROM public.ordres_fabrication o, jsonb_array_elements(COALESCE(o.technical_snapshot->'preparation_evidence'->'purchases','[]')) p
        WHERE o.id=$1 AND p->>'type_achat'='MATIERE' AND($2::uuid IS NULL OR p->>'article_id'=$2::text))) AS guarded`,[ofId,articleId])).rows[0]?.guarded;
  if(guarded)throw new HttpError(409,"OF_MATERIAL_COVERAGE_REQUIRED","Ouvrez « Matière et achats » dans le dossier OF pour vérifier la couverture et préparer uniquement le manque.",{of_id:ofId});
}
