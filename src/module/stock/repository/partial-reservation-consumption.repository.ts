...
export async function consumeMaterialReservationTx(tx:PoolClient,input:Input,audit:AuditContext):Promise<Result>{
  ...
  const row=(await tx.query<{
  ...
  }>(`SELECT r.article_id::text,r.lot_id::text,r.stock_batch_id::text,s.id::text AS stock_level_id,e.magasin_id::text,e.id::int AS emplacement_id,a.unite AS unit,
    ...
    WHERE r.id=$1::uuid AND r.of_id=$2 AND n.of_id=$2 AND n.need_kind=$4 AND n.superseded_at IS NULL
      AND(($4='MATIERE' AND n.operation_id=$3::uuid AND b.id IS NOT NULL)
    ...
    FOR UPDATE OF r`,[input.reservationId,input.ofId,input.operationId,input.kind??'MATIERE'])).rows[0];
  ...
  const created=await repoCreateMovement({movement_type:'OUT',source_document_type:'OF',source_document_id:String(input.ofId),reason_code:input.kind==='CONSOMMABLE'?'PRELEVEMENT_CONSOMMABLE':'DEBIT_MATIERE',notes:input.reason,
