...
exports.outilRepository = void 0;
const database_1 = __importDefault(require("../../../config/database"));
exports.outilRepository = {
    // 🔍 Trouver un outil par son ID
    async findById(id) {
        const result = await database_1.default.query("SELECT * FROM gestion_outils_outil WHERE id_outil = $1", [id]);
        return result.rows[0] || null;
    },
    // ➕ Créer un nouvel outil (version simplifiée - version étendue via service)
    async create(data) {
        const result = await database_1.default.query(`INSERT INTO gestion_outils_outil (
    ...
    // 📦 Familles outils
    async getFamilles() {
        const result = await database_1.default.query("SELECT id_famille, nom_famille FROM gestion_outils_famille");
        return result.rows.map(row => ({ value: row.id_famille, label: row.nom_famille }));
    },
    // 🏭 Fabricants
    async getFabricants() {
        const result = await database_1.default.query("SELECT id_fabricant, name FROM gestion_outils_fabricant");
        return result.rows.map(row => ({ value: row.id_fabricant, label: row.name }));
    },
    async createFabricant(nom_fabricant, logo, id_fournisseurs) {
        const client = await database_1.default.connect();
        try {
            await client.query("BEGIN");
            const result = await client.query(`INSERT INTO gestion_outils_fabricant (name, logo) VALUES ($1, $2) RETURNING id_fabricant`, [nom_fabricant, logo]);
            const id_fabricant = result.rows[0].id_fabricant;
            for (const id_fournisseur of id_fournisseurs) {
                await client.query(`INSERT INTO gestion_outils_fournisseur_fabricant (id_fabricant, id_fournisseur) VALUES ($1, $2)`, [id_fabricant, id_fournisseur]);
            }
    ...
    // 🤝 Fournisseurs
    async getFournisseurs(fabricantId) {
        if (fabricantId) {
            const result = await database_1.default.query(`SELECT f.id_fournisseur, f.nom
         ...
         WHERE ff.id_fabricant = $1`, [fabricantId]);
            return result.rows.map(row => ({ value: row.id_fournisseur, label: row.nom }));
        }
        else {
            const result = await database_1.default.query("SELECT id_fournisseur, nom FROM gestion_outils_fournisseur");
            return result.rows.map(row => ({ value: row.id_fournisseur, label: row.nom }));
        }
    },
    async createFournisseur(data) {
        const { nom, adresse_ligne, house_no, postcode, city, country, phone_num, email } = data;
        await database_1.default.query(`INSERT INTO gestion_outils_fournisseur 
        (nom, adresse_ligne, house_no, postcode, city, country, phone_num, email) 
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [nom, adresse_ligne, house_no, postcode, city, country, phone_num, email]);
    },
    async getGeometries(id_famille) {
    ...
    async getRevetements(id_fabricant) {
    ...
    async getAretes(id_geometrie) {
        let result;
        if (id_geometrie) {
            result = await database_1.default.query(`SELECT gac.id_arete_coupe, ac.nom_arete_coupe
         ...
         ORDER BY ac.nom_arete_coupe`, [id_geometrie]);
        }
        else {
            result = await database_1.default.query(`SELECT id_arete_coupe, nom_arete_coupe FROM gestion_outils_arete_coupe ORDER BY nom_arete_coupe`);
        }
